...
func (gr *GlobalRolesV2TestSuite) TestUserWithInheritedClusterRolesImpactFromClusterDeletion() {
	...
	credentialSpec := cloudcredentials.LoadCloudCredential(string(provider.Name))
	machineConfigSpec := machinepools.LoadMachineConfigs(string(provider.Name))
	clusterObject, err := provisioning.CreateProvisioningCluster(gr.client, provider, credentialSpec, gr.clusterConfig, machineConfigSpec, nil)
	require.NoError(gr.T(), err)
	provisioning.VerifyCluster(gr.T(), gr.client, clusterObject)
	...
	err = clusterapi.DeleteK3SRKE2Cluster(userClient, clusterObject.ID)
	require.NoError(gr.T(), err)
